...
function register($data)
{
	...
	if ($password !== $password2) {
		$checkPassMatch = true;
		return false;
	} elseif (strlen(trim($password)) < 5 && !preg_match('/[A-Z]/', $password)) {
		$checkPass2 = true;
		return false;
		...
		mysqli_query(
			$conn,
			"INSERT INTO users VALUES('','$name', '$username','$bio', '$password', '$position','$instagram','$github','$country','$birthday', '$age', '$image')"
		);
		...
		while ($row = mysqli_fetch_assoc($result)) {
			if (password_verify($password, $row['password'])) {
				if ($password1 != $password2) {
					...
					(strlen(trim($password1)) < 5 &&
						!preg_match('/[A-Z]/', $password1)) ||
					(strlen(trim($password2)) < 5 && !preg_match('/[A-Z]/', $password2))
				) {
					$checkPass2 = true;
					return false;
				} elseif (
...
function addTicket($data)
{
	...
	mysqli_query(
		$conn,
		"INSERT INTO ticket VALUES('','$ticket_title','$date_ticket', '$creator','$desc_ticket', '$status_ticket', '$image', '$id_user','$solved_by','$feedback')"
	);
...
function confirmTicket($data)
{
	...
	mysqli_query(
		$conn,
		"UPDATE ticket SET status_ticket = '$statusTciket' WHERE id = '$id'"
	);
	return mysqli_affected_rows($conn);
}
